...
from database import collection_account, collection_transaction, collection_predicted_income, collection_predicted_expense, collection_user, collection_predicted_balance,collection_dummy_values,collection_Todo_list, collection_chatbot_details
...
# }
    ...
    system_prompt="""you are a classification assistant. Your task is to classify given user query into one of following two categories quickly:
        ...
        before responding, you must check Non to-do list task and To-do list task fields in JSON and make sure both are not empty at same time.if empty add user query to "Non to-do list task" field.
...
you are not allowed to give empty sanitize_output, if it is empty give user output as it is.

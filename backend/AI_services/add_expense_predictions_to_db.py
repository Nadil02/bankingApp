...
from models import PredictedExpense 
from database import collection_predicted_expense
import pandas as pd
async def insert_predicted_expense_from_excel():
    # Read the Excel file
    df = pd.read_excel(r"C:\Users\User\Downloads\tft_predictions_with_details_explanations_shihara_expense.xlsx")

    # Get the maximum prediction_id from the collection
    # max_prediction_id = await collection_predicted_expense.find_one(
    ...
    for index, row in df.iterrows():
        document = PredictedExpense(
            account_id=3,
            date=pd.to_datetime(row['Date']),
            explanation=row['explanation'],
            amount=float(row['Amount_Prediction']),
            category_id=int(row['Cluster']),
            user_id=1,
            clasification_uncertainity=float(row['Occurrence_Probability'])*100,
            regression_uncertainity=float(row['Amount_Uncertainty'])
        )
        documents.append(document.dict())

    # Insert the documents into the collection
    if documents:
        await collection_predicted_expense.insert_many(documents)
if __name__ == "__main__":
    import asyncio
    asyncio.run(insert_predicted_expense_from_excel())
